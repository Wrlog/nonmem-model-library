...
Control streams for techniques that come up in real analyses but do not fit
the `models/` pattern of one simulator and one estimator per model. They are
templates: no dataset ships with them, and `$INPUT` in each one lists the
columns it expects. Point `$DATA` at your own file.

| Example | What it shows |
| --- | --- |
| [`pk_1cmt_oral_m3_blq.mod`](pk_1cmt_oral_m3_blq.mod) | Oral one-compartment model with a lag time and combined error, and the **M3 method** for records below the limit of quantification: they contribute `P(C < LLOQ)` rather than being dropped |
...
Each one was run in NONMEM 7.5 on data simulated from known parameter values
before it was added here, and recovered them. That run is not repeated in CI,
since it needs a NONMEM licence. What CI does run on every example is the
